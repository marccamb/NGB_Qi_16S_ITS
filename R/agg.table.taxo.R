...
#' a given taxonomic level, found in each sample.
#' @import stats
#'
# 2019-11-28
# Marine Cambon
...
agg.table.taxo <- function(tab, tax.lvl="genus", tax.table) {
  if (any(rownames(tab) != rownames(tax.table))) stop("The ASV/OTU table and the taxonomy table are ordered the same way")
